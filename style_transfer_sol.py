...
class StyleTransfer(object):
    def __init__(self, content_img, style_img, img_width, img_height):
        ...
        self.gstep = tf.compat.v1.Variable(0, dtype=tf.compat.v1.int32, 
    ...
    def create_input(self):
        ...
        with tf.compat.v1.variable_scope('input') as scope:
            self.input_img = tf.compat.v1.get_variable('in_img', 
    ...
    def _content_loss(self, P, F):
        ...
        ###############################
        ## TO DO
        self.content_loss = tf.compat.v1.reduce_sum((F - P) ** 2) / (4.0 * P.size)
        ###############################
    ...
    def _gram_matrix(self, F, N, M):
        ...
        ###############################
        ## TO DO
        F = tf.compat.v1.reshape(F, (M, N))
        return tf.compat.v1.matmul(tf.compat.v1.transpose(F), F)
        ###############################
    ...
    def _single_style_loss(self, a, g):
        ...
        return tf.compat.v1.reduce_sum((G - A) ** 2 / ((2 * N * M) ** 2))
        ###############################
    ...
    def losses(self):
        with tf.compat.v1.variable_scope('losses') as scope:
    ...
    def optimize(self):
        ...
        self.opt = tf.compat.v1.train.AdamOptimizer(self.lr).minimize(self.total_loss,
    ...
    def create_summary(self):
        ...
        with tf.compat.v1.name_scope('summaries'):
            tf.compat.v1.summary.scalar('content loss', self.content_loss)
            tf.compat.v1.summary.scalar('style loss', self.style_loss)
            tf.compat.v1.summary.scalar('total loss', self.total_loss)
    ...
    def train(self, n_iters):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            writer = tf.compat.v1.summary.FileWriter('projects\styleTransfer2019', sess.graph)
            ###############################
            ...
            ckpt = tf.compat.v1.train.get_checkpoint_state(os.path.dirname('projects\styleTransfer2019\checkpoint'))
...
if __name__ == '__main__':
    setup()
    machine = StyleTransfer('content/1.jpg', 'styles/harlequin.jpg', 1930, 2500)
    machine.build()
    machine.train(300)
